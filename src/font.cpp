...
void initFonts(int windowWidth, int windowHeight) {
    ...
    GLuint vs = createVertexShader("shaders/vert_text.glsl");
    GLuint fs = createFragmentShader("shaders/frag_text.glsl");

    glAttachShader(fontProgram, vs);
	glAttachShader(fontProgram, fs);

    glBindAttribLocation(fontProgram, 2, "vertex");

    glLinkProgram(fontProgram);
    GLint status;
	glGetProgramiv(fontProgram, GL_LINK_STATUS, &status);
